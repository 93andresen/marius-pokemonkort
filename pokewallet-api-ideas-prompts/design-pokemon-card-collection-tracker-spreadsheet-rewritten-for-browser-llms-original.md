...
I am making a **Pokémon Card Collection Tracker in a Google Spreadsheet** and I need a comprehensive, well-thought-through **design plan** — no implementation yet.

## Your role in this conversation (important — please read)

You are being asked to **produce a written plan only**. In this chat you have **no tools, no code execution, and no API access** — so please do **not** try to run, test, or "verify" anything. Base your plan purely on the information in this prompt (including the full API documentation and the full Google Sheets tool definitions included below) plus your own knowledge of spreadsheets, data modeling, and the Pokémon TCG.

## Context: who will actually build this

The plan you produce will be handed to an **implementation agent** that *does* have these capabilities:

- A **Google Sheets MCP server** — it can read/write spreadsheet tabs, cells, and formulas directly (exact tool definitions below in Appendix B).
- **Code execution** — it can run scripts to call the API, parse data, and build import pipelines.
- A **PokeWallet API key** stored in the environment as a persistent env var named `API_KEY_POKEWALLET`, which it can use to test endpoints live.

So: design for that agent to implement. You may reference concrete API endpoints, spreadsheet formulas, tab schemas, and workflows. Where you are unsure about exact API behavior, you may mark things as *"implementing agent should verify this live with the key"*. But **you yourself** will not be doing any implementation.
...
Just in general: everything I want to do as a Pokémon card collector. Specific requirements to design for:

- **Price paid for each card** — and specifically, how to resolve these hard cases:
  - **Bulk purchases:** how do you attribute cost when many cards are bought together (lots, binders, sealed deals)?
  - **Differential price changes:** when cards bought together later change price at different rates, how should cost basis be handled, and what gets credited to which card?
  - **The different ways to think about this, and the trade-offs of each.**
- **Bulk cost allocation must be explicit and auditable** — it should always be answerable *why* a card has the cost basis it does.
- **Price history:** how to track prices over time given that official price-history endpoints are Pro-only (see docs).
- **Current-price tracking** within the API rate-limit budget (**100 requests/hour, 1000 requests/day** — this is a first-class constraint).
- **Individual card entry:** adding single cards conveniently (type a card name → auto-fetch data from the API).
- **Scanner app exports:** I use scanners for many cards and will export lists from scanner apps (bulk CSV/Excel-style imports). The design must tolerate messy external data and dedupe/import it cleanly. This is a later phase, but keep it in mind when designing the system.
- **Different views/dashboards:** I've been thinking about different views, but first we need the core system working so cards can be added optimally and as conveniently as possible.

Feel free to include different suggestions in your plan. I am open to anything useful.

## Output

Produce a **complete markdown document** with a comprehensive, well-thought-through plan for creating this spreadsheet. Cover:

1. The optimal spreadsheet structure and **why** (tab-by-tab schema, key columns, relationships between tabs).
2. Bulk purchase cost-allocation methods (multiple approaches, trade-offs, a recommendation, and a worked example).
3. Price tracking over time and current-price caching strategy, within the rate limits.
4. Workflows for adding cards (individual entry + scanner exports).
5. Dashboards/views for the collector use cases.
6. Useful formulas, conventions, and implementation notes for the implementing agent.
...
# Appendix A — Full PokeWallet API Documentation

> Everything below is the complete, unmodified API documentation for the data source. Use it to make your plan concrete and correct. Note the Pro-only endpoints (the implementing agent is on the **Free plan**: 100 req/hour, 1000 req/day).
...
# Appendix B — Google Sheets MCP: Tools the Implementation Agent Has

> The implementation agent connects to Google Sheets through an MCP server with the following tools. Design your plan so it can be executed with exactly these operations.

## 1. `list_sheets` — List all sheets (tabs) in the spreadsheet

**Parameters:**
- `spreadsheet_id` (string, required): The ID of the spreadsheet.

Returns the names of all sheet tabs in the spreadsheet. Use this to discover what already exists before creating anything.

## 2. `get_sheet_data` — Get data from a specific sheet

**Parameters:**
- `spreadsheet_id` (string, required): The ID of the spreadsheet.
- `sheet` (string, required): The name of the sheet tab.
- `range` (string, optional): Cell range in A1 notation, e.g. `A1:C10`. If omitted, the whole sheet.
- `include_grid_data` (boolean, optional, default `false`): If `true`, includes cell formatting and metadata.

Use this to read existing rows, verify writes, and inspect what's in the spreadsheet.

## 3. `update_cells` — Update cells in a sheet

**Parameters:**
- `spreadsheet_id` (string, required): The ID of the spreadsheet.
- `sheet` (string, required): The name of the sheet tab.
- `range` (string, required): Cell range in A1 notation, e.g. `A1:D5`.
- `data` (2D array of values, required): The values to write. Each inner array is a row.

Use this to write card data, prices, formulas, etc. into the spreadsheet.

## 4. `batch_update_cells` — Update multiple ranges in one call

**Parameters:**
- `spreadsheet_id` (string, required): The ID of the spreadsheet.
- `sheet` (string, required): The name of the sheet tab.
- `ranges` (object, required): A dictionary mapping range strings (A1 notation) to 2D arrays of values.

Use this to write several non-contiguous ranges at once — efficient for writing headers + data in a single operation.

## 5. `create_sheet` — Create a new sheet tab

**Parameters:**
- `spreadsheet_id` (string, required): The ID of the spreadsheet.
- `title` (string, required): The title for the new sheet tab.

Use this to create each tab in your design (e.g. `Cards`, `Purchases`, `Prices`, `Dashboard`, etc.).

---

> **Important capabilities note:** the MCP server can read and write cells, tabs, and (via `update_cells`/`batch_update_cells`) cell contents — which includes entering **formulas** as cell values. Combined with the implementation agent's **code execution** (Python scripts for API calls, parsing, and imports), anything you design in this plan is implementable. There is no practical limit on the number of tabs, rows, or formulas.
